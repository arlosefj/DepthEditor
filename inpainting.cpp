...
double inpainting::ComputeData(int i, int j)
{
	...
	double max=0;
	int x, y;
	for(y = H3D_MAX(j -winsize,0); y<= H3D_MIN(j+winsize,m_height-1); y++)
		for( x = H3D_MAX(i-winsize,0); x<=H3D_MIN(i+winsize, m_width-1); x++)
		{
...
void inpainting::UpdatePri(int i, int j) // just update the area near the changed patch. (+-3 pixels)
{
	...
	for(y = H3D_MAX(j -winsize-3,0); y<= H3D_MIN(j+winsize+3,m_height-1); y++)
		for( x = H3D_MAX(i-winsize-3,0); x<=H3D_MIN(i+winsize+3, m_width-1); x++)if(m_mark[y*m_width+x] == BOUNDARY)m_pri[y*m_width+x] = priority(x,y);
