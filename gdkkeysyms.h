...
/*
GTK检测窗体中组合键,这个困扰了我好长时间的问题,今天终于解决了,总结一下:

定义全局变量标识热键状态
boolean Shift_press_flag = FALSE;
boolean Ctrl_press_flag = FALSE;
boolean Alt_press_flag = FALSE;

为窗体的key_press_event事件添加回调函数
g_signal_connect (G_OBJECT (window), "key-press-event", G_CALLBACK (key_press_event_fun), NULL);

gboolean key_press_event_fun(GtkWidget * widget, GdkEventKey *event, gpointer data){
 if (event->keyval==0xFFE1 || event->keyval== 0xFFE2){
  ...
  return FALSE;
                ...
                Shift_press_flag = TRUE;               //Alt键按下
  return FALSE;
        }
 
 while (Ctrl_press_flag){
  if (event->keyval== 0x064 || event->keyval== 0x044){
   g_print("Ctrl+d pressed!);     // d  0x064  D  0x044
   /* add ctrl_d code */
   return TRUE;
  }
 }       
...
为窗体的key_release_event事件添加回调函数
g_signal_connect (G_OBJECT (window), "key_release_event", G_CALLBACK (key_release_event_fun), NULL);

gboolean key_release_event_fun(GtkWidget * widget, GdkEventKey *event, gpointer data){
 if (event->keyval==0xFFE1 || event->keyval== 0xFFE2){
